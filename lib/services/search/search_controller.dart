...
import 'package:the_disease_fighter/services/search/search_model.dart';
...
class SearchController {
  ...
  Future _getSearch({searchText}) async {
    ...
    var response = await _dio.get('/doctors/search?name=${searchText.toString()}');
  ...
  Future<SearchModel?> loadSearchResult({docName}) async {
    var jsonString, jsonResponse;
    jsonString = await _getSearch(searchText: docName);
    jsonResponse = json.decode(jsonString.toString());
    _searchModel = SearchModel.fromJson(jsonResponse);
    return _searchModel;
  }
}
